...
# 2. ActiveMQ is running on the right ports
# 3. Solr and Sesame are running
# 4. Alveo Workers are running
# 5. Alveo HcsVlab is running
# 6. RabbitMQ is running on the right port
#
...
HCSVLAB_URL="http://localhost:3000/version.json"
...
then
  HCSVLAB_URL="https://localhost/version.json"
...
do
  sleep 2
  web_status=`curl -Ik ${HCSVLAB_URL} 2>/dev/null  | head -1 | awk '{print $2}' `
  let count=count+1
done
